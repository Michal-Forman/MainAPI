...
export const verifyToken = (req, res, next) => {
  ...
  if (!token) return res.sendStatus(401); // It's more common to check for falsy values like this

  jwt.verify(
    token,
    process.env.SMART_DIET_TRACKER_JWT_SECRET,
    async (err, payload) => {
      if (err) return res.sendStatus(403); // Forbidden if there's an error verifying the token
